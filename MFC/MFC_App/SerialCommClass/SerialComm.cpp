...
DWORD SerialComm::GetSendBufferSize() const
{
	return m_nSendBufferSize;
}

DWORD SerialComm::GetRecvBufferSize() const
{
	return m_nRecvBufferSize;
}

DCB SerialComm::GetDcb() const
{
	return m_dcb;
}

DWORD SerialComm::GetCommEvent() const
{
...
/************************************************************************/
bool SerialComm::Init(CWnd *pPortOwner,										//主窗口句柄
					  UINT portnr /* = 1 */,								//串口号
...
/************************************************************************/
bool SerialComm::SetRate(HANDLE &hCom, UINT nRate)
{
	if (NULL != hCom)
	{
		DCB dcb;
...
/************************************************************************/
bool SerialComm::SendData(SerialComm *port)
{
...
/************************************************************************/
bool SerialComm::RecvData(SerialComm *port, COMSTAT &comstat)
{
	DWORD dwBytesRead = 0;
	DWORD dwErr = 0;
	char *buf = new char[port->m_nRecvBufferSize];
	char *rxbuf = new char[256];
	...
	while (1)
	{
		...
		//接收数据
		if (!ReadFile(port->m_hCom, rxbuf, 256, &dwBytesRead, &port->m_ov))
		{			
			if (ERROR_IO_PENDING != GetLastError())
			{	//操作出错,离开临界区
				delete [] buf;
	...
	if ("" != buf)
	{
		...
		::PostMessage(port->m_pMainWnd->m_hWnd, WM_COMM_RXCHAR, (WPARAM)buf, (LPARAM)port->m_nPortNr);
		delete [] buf;
		//离开临界区
		LeaveCriticalSection(&port->m_cs);
	}
...
// return														
/************************************************************************/
UINT SerialComm::CommThread(LPVOID pParam)
{
	DWORD dwErr = -1;
	DWORD dwEvtMask = 0;
	COMSTAT comstat = {0};
	...
	SerialComm *port = (SerialComm *)pParam;
