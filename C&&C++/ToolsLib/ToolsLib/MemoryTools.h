...
typedef struct _TOOLS_MEM_BLOCK_HEAD_
{
	...
	struct _TOOLS_MEM_BLOCK_HEAD_* m_pNext; //指向下一链表元素的指针
...
//本类使用了一定的递归逻辑
class TOOLSLIB_API CToolsMemoryStackToken
{ 
public:
...
//内存栈
class TOOLSLIB_API CToolsMemoryStack
{ 
public:
	CToolsMemoryStack(CToolsLowDebug* pDebug); //构造函数，需要传入Debug 对象
	~CToolsMemoryStack();
public:
	...
	CToolsLowDebug* m_pDebug; //debug 对象，此处为了优化公开
private:
	CToolsMemoryStackToken* m_pHead; //注意，这是左枝开始的第一个节点
...
{
	void* m_pPoint; //保留的指针
	char m_szInfo[TOOLS_MEMORY_BLOCK_INFO_MAX_SIZE]; //说明文字
}SToolsMemoryRegister; //定义的变量类型
//习惯性写法，声明了结构体后，立即定义其尺寸常量
const ULONG SToolsMemoryRegisterSize=sizeof(SToolsMemoryRegister);

//内存注册类
class TOOLSLIB_API CMemoryRegister
{
public:
	CMemoryRegister(CToolsLowDebug* pDebug); //构造函数传输debug 对象指针
	~CMemoryRegister();
public:
	//添加一个指针及其说明文字
...
private:
	CToolsLowDebug* m_pDebug; //Debug 对象
...
//内存池类
class TOOLSLIB_API CToolsMemoryPoolWithLock
{ 
public:
	//构造函数
	//考虑到某些成熟的代码模块，已经经过测试，确定没有bug，可以考虑在这里
	//关闭注册功能，这样，减少了每次内存申请和释放时，注册和反注册的逻辑，可以提升效率
	CToolsMemoryPoolWithLock(CToolsLowDebug* pDebug, //传入的Debug 对象指针
...
public:
	...
	//关键信息显示
	void PrintInfo(void);
	CToolsMemoryStack* m_pMemPool; //内存栈对象
