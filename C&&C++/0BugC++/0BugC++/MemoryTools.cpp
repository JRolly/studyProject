	...
	CMRSWint& nMemoryUse)
{
	...
	SToolsMemoryBlockHead* pNew=null; //中间变量
	//请注意这个宏的使用
...
bool CToolsMemoryStackToken::Free(void* pPoint,bool bCloseFlag)
{
	...
	if(m_ulBlockSize==pOld->m_ulBlockSize)
	{
		...
		{ //当CloseFlag 为真，不再推入右枝，直接释放
			free(pOld);
			m_nBlockOutSide.Dec();
		} 
		else
		{ //当所有条件不满足，正常推入右枝栈，准备重用内存块
			m_Lock.EnableWrite(); //加写锁
...
{ //本函数是典型的对象内私有函数，被其他线程安全的函数调用，因此，内部不加锁。
	SToolsMemoryBlockHead* pObjNow=pSon; //中间变量
...
CToolsMemoryStack::CToolsMemoryStack(CToolsLowDebug* pDebug)
{
	...
	m_nMemoryUse.Set(0);
	m_pHead=new CToolsMemoryStackToken( //以最小内存块尺寸16Bytes，
		TOOLS_MEMORY_STACK_BLOCK_MIN, //构造左枝第一个节点
		m_pDebug);
...
//并由此获得原有尺寸的真实大小。
void* CToolsMemoryStack::ReMalloc(void* pPoint, //原有指针，p1
	int nNewSize, //新尺寸
	bool bCopyOldDataFlag) //拷贝旧数据标志
{
	void* pRet=null;
	SToolsMemoryBlockHead* pOldToken=null; //旧的p0
	int nOldLen=0; //旧的长度
...
void CMemoryRegister::RegisterCopy(
	SToolsMemoryRegister* pDest, //目标结构体指针
	...
	char* szInfo) //待拷贝的说明文字
{
	...
	{ 
		...
		TOOLS_CLEAN_CHAR_BUFFER(szInfo);//如果为空，则缓冲区置为空字符串
}
...
void CMemoryRegister::Add(void* pPoint,char* szInfo)
{
	...
	m_Lock.Lock(); //加锁
	{
		...
		if(TOOLS_MEMORY_REGISTER_MAX<=m_nUseMax)
		{
			...
			//增加TOOLS_MEMORY_REGISTER_MAX 的值。
...
{
	m_pDebug=pDebug; //保存debug 对象指针
	m_pMemPool=new CToolsMemoryStack(m_pDebug); //实例化内存栈对象
